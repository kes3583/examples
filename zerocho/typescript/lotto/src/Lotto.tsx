...
function getWinNumbers() {
  ...
  const candidateNumbers = Array(45).fill(null).map((el, i) =>  i + 1) //null 필수
  const shuffle = []
...
function Lotto () {
  ...
  const lottoNumbers = useMemo(() => getWinNumbers(), []); //useMemo - 두번째 인자가 안바뀌면 getWinNumbers를 불러오지 않는다. 함수의 리턴 값을 기억한다. useMemo는 리턴값을 기억하고 , useCallback은 리턴함수 자체를 기억하여 컴포넌트가 재실행되도 함수는 재실행되지않는다. 
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < winNumbers.length - 1; i++) {
      ...
      timeouts.current[i] = window.setTimeout( () => { // settimeout 은 arrow function 사용 , timeouts.current[i]는 timeouts.current가 바뀌는게 아니다. 단순히 current에 배열추가. 
